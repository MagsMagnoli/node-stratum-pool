        ...
        for (var r in options.rewardRecipients){
            ...
            try {
                if (r.length === 40)
                    ...
                    rObj.script = util.addressToScript(util.addressFromEx(r));
                else
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            options.poolAddressScript = (function(){
                switch(options.coin.reward){
                    ...
                    case 'POW':
                        const address = rpcResults.validateaddress.address;
                        if(address.length > 40) {
                            return util.addressToScript(util.addressFromEx(address));
                        }
                        return util.addressToScript(address);
                }
            })();
